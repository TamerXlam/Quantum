...
void AALSBaseCharacter::SetupPlayerInputComponent(UInputComponent* PlayerInputComponent)
{
	...
	PlayerInputComponent->BindAction("Walk", IE_Released, this, &AALSBaseCharacter::WalkReleasedAction);
	...
	PlayerInputComponent->BindAction("Aim", IE_Pressed, this, &AALSBaseCharacter::AimPressed);
	PlayerInputComponent->BindAction("Aim", IE_Released, this, &AALSBaseCharacter::AimReleased);
	PlayerInputComponent->BindAction("ChangeView", IE_Pressed, this, &AALSBaseCharacter::CameraPressedAction);
	PlayerInputComponent->BindAction("ChangeView", IE_Released, this, &AALSBaseCharacter::CameraReleasedAction);
	PlayerInputComponent->BindAction("JetPack", IE_Pressed, this, &AALSBaseCharacter::JetPressedAction);
	PlayerInputComponent->BindAction("JetPack", IE_Released, this, &AALSBaseCharacter::JetReleasedAction);
	PlayerInputComponent->BindAction("Interact", IE_Pressed, this, &AALSBaseCharacter::InterractPressedAction);
	PlayerInputComponent->BindAction("Interact", IE_Released, this, &AALSBaseCharacter::InterractReleasedAction);
}
...
void AALSBaseCharacter::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	...
	DOREPLIFETIME(AALSBaseCharacter, MovementState);
	...
	DOREPLIFETIME(AALSBaseCharacter, Team);
	DOREPLIFETIME(AALSBaseCharacter, IsAimed);
	DOREPLIFETIME(AALSBaseCharacter, HealthValue);
	DOREPLIFETIME(AALSBaseCharacter, CameraMode);
	DOREPLIFETIME(AALSBaseCharacter, TargetCameraMode);
}
...
void AALSBaseCharacter::WalkReleasedAction()
{

}

void AALSBaseCharacter::OnFixedAimViewModeChanged(const EALSViewMode PreviousViewMode)
{
	//MainAnimInstance->GetCharacterInformationMutable().FixedViewMode = FixedAimViewMode;
...
void AALSBaseCharacter::OnJetPackActivated_Implementation()
{
	///
}
void AALSBaseCharacter::OnJetPackDeactivated_Implementation()
{
	///
}
...
void AALSBaseCharacter::OnHealthChanged(float PrevHealth)
{

}

void AALSBaseCharacter::OnIsAimedChanged(bool PreviousIsAimed)
{

}

void AALSBaseCharacter::HealthChanged_BP_Implementation()
{

	//
}

void AALSBaseCharacter::Server_DOHealthChanged_Implementation(float DamagePower)
{
...
void AALSBaseCharacter::Multicast_Die_Implementation()
{
...
void AALSBaseCharacter::OnOverlayStateChanged(const EALSOverlayState CurrentState)
{
	MainAnimInstance->OverlayState = OverlayState;
...
void AALSBaseCharacter::Server_SetIsAimed_Implementation(bool NewAimed)
{
	IsAimed = NewAimed;
}

void AALSBaseCharacter::OnViewModeChanged_Native_Implementation(EALSViewMode NewViewMode)
{

	//
}

void AALSBaseCharacter::Multicast_OnViewModeChanged_Implementation(EALSViewMode NewViewMode)
{
	OnViewModeChanged_Native(NewViewMode);
}
...
void AALSBaseCharacter::Server_AimPressedAction_Implementation()
{
	AimPressedAction();
}
...
void AALSBaseCharacter::JetPressedAction()
{
	UWorld* World = GetWorld();
	check(World);
	JetPressedTime = World->GetTimeSeconds();
	GetWorldTimerManager().SetTimer(OnJetTimer, this,
		&AALSBaseCharacter::OnJetPack, JetHoldTime, false);
}

void AALSBaseCharacter::JetReleasedAction()
{

	UWorld* World = GetWorld();
	check(World);
	if (World->GetTimeSeconds() - JetPressedTime < JetHoldTime)
	{
		// Switch shoulders

		GetWorldTimerManager().ClearTimer(OnJetTimer); // Prevent mode change
	}

}

void AALSBaseCharacter::InterractPressedAction()
{

	///
	AActor* MyParent = GetAttachParentActor();
	if (IsValid(MyParent))
	{
		//Multicast_InterractPressed
		//UsingAnotherPawn_BP(nullptr);
	}
...
void AALSBaseCharacter::Multicast_InterractPressed_Implementation()
{

}

void AALSBaseCharacter::InterractReleasedAction()
{
...
void AALSBaseCharacter::OnFPCameraMode()
{

	SetTargetCameraMode(EALSViewMode::FirstPerson);
	SetCameraMode(EALSViewMode::FirstPerson);
	SetRotationMode(EALSRotationMode::Aiming);
	SetViewMode(EALSViewMode::FirstPerson);

}

void AALSBaseCharacter::OnTPCameraMode()
{

	SetTargetCameraMode(EALSViewMode::ThirdPerson);
	SetCameraMode(EALSViewMode::ThirdPerson);
	SetRotationMode(EALSRotationMode::LookingDirection);
	SetViewMode(EALSViewMode::ThirdPerson);
}

void AALSBaseCharacter::OnJetPack()
{

	if (CameraMode == EALSViewMode::FirstPerson)
	{
	...
	else if (CameraMode == EALSViewMode::ThirdPerson)
	{
...
void AALSBaseCharacter::OnRep_HealthValue(float PrevHealth)
{
	OnHealthChanged(PrevHealth);
}

void AALSBaseCharacter::OnRep_IsAimed(bool PrevAimed)
{
	OnIsAimedChanged(PrevAimed);
}

void AALSBaseCharacter::OnRep_MovementAction(EALSMovementAction PrevRotMode)
{
	OnMovementActionChanged(PrevRotMode);
}
...
void AALSBaseCharacter::OnRep_MovementState(EALSMovementState PrevMovState)
{
	OnMovementStateChanged(PrevMovState);
}
...
void AALSBaseCharacter::OnRep_CameraMode(EALSViewMode PrevViewMode)
{
	//OnCameraModeChanged(PrevViewMode);
}

void AALSBaseCharacter::OnRep_TargetCameraMode(EALSViewMode PrevViewMode)
{
//	OnTargetCameraModeChanged(PrevViewMode);
}

void AALSBaseCharacter::OnCameraModeChanged(const EALSViewMode PreviousState)
{
...
void AALSBaseCharacter::OnTargetCameraModeChanged(const EALSViewMode PreviousState)
{
